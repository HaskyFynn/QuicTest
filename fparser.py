...
from my_util import get_varint
...
def parse_stream_frame(packet_bytearray: bytearray, payload_offset: int) -> Tuple[dict, int]:
    ...
    if (first & 0x02) != 0:
        ...
        # Validate frame length against remaining packet data
        if payload_offset + frame_len > len(packet_bytearray):
            print(f"Error: Frame length ({frame_len}) + payload offset ({payload_offset}) exceeds packet length ({len(packet_bytearray)})")
            # Consider raising a specific error or handling incomplete packets here
